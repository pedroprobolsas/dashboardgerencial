...
// Replace handlePrint inside ReporteCierreSiigo
const newHandlePrint = `
  const handlePrint = async () => {
    ...
    const createHTML = (title, instructions, docNo, tableRows, totalsHTML, observation) => \`
      <!DOCTYPE html>
      <html>
      <head>
        <meta charset="utf-8">
    ...
    saveAs(content, \`Comprobantes-Cierre-\${year}-\${String(month).padStart(2, '0')}.zip\`);
  };
`;

const printRegex = /const handlePrint = \(\) => \{[\s\S]*?printWindow\.document\.close\(\);\n  \};/m;
content = content.replace(printRegex, newHandlePrint.trim());
