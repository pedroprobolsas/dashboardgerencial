...
export default function KPICard({ kpi }: { kpi: KPI }) {
  ...
  return (
    <div className={`bg-white rounded-3xl shadow-sm border ${colores.borde} p-5 flex flex-col gap-3 transition-shadow hover:shadow-md relative`}>
      {/* Icono de impresión (Opcional) */}
      {kpi.onClickReporte && (
        <button 
          onClick={kpi.onClickReporte}
          className="absolute top-4 right-4 p-1.5 text-slate-400 hover:text-probolsas-cyan hover:bg-slate-50 rounded-md transition-colors"
